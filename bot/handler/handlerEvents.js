        ...
        async function onStart() {
            ...
            if (needRole > role) {
                if (!hideNotiMessage.needRoleToUseCmd) {
                    ...
                    else if (needRole == 2) return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdminBot", commandName));
                }
                return true;
            }
            ...
            // ✅ Execute the command
            try {
                await command.onStart({ ...parameters, args, message, event });
            } catch (err) {
                log.error(err);
                await message.reply(`❌ Error while executing command "${commandName}": ${err.message}`);
            }
        }
